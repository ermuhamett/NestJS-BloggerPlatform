...
import {CommentLikes, CommentLikesDocument} from "../../../likes/domain/like.entity";
import {CommentMapper} from "../api/models/output/comment.output.model";
import {LikesInfo, LikeStatus} from "../../../likes/api/models/likes.info.model";
...
export class CommentQueryRepository {
    constructor(@InjectModel(Comment.name) private commentModel: Model<CommentDocument>,
                @InjectModel(CommentLikes.name) private commentLikesModel: Model<CommentLikesDocument>) {
    }
    ...
    async getCommentLikes(commentId: string, userId?: string) {
        try {
            ...
            if (userId) {
                userLike = await this.commentLikesModel.findOne({parentId: commentId, authorId: userId}).lean()
                if (userLike) {
            ...
            const [likesCount, dislikesCount] = await Promise.all([
                this.commentLikesModel.countDocuments({parentId: commentId, status: LikeStatus.LIKE}),
                this.commentLikesModel.countDocuments({parentId: commentId, status: LikeStatus.DISLIKE})
            ])
            return {
                likesCount,
                dislikesCount,
    ...
    async getCommentsWithPaging(query: QueryOutputType, postId?: string, userId?: string) {
        ...
        try {
            ...
            const extendedLikesInfos = await Promise.all(commentsIds.map(commentId => this.getCommentLikes(commentId, userId)))
            ...
            return{
                ...
                items:commentsWithLikeStatus.map(item=>CommentMapper.toView(item.comment, item.extendedLikesInfo))
            }
        }catch (e) {
